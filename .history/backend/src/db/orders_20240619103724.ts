...
Order.init(
  {
    ...
    status: {
      type: DataTypes.ENUM,
      values: ['pending','validated','pending delivery', 'delivery', 'delivered', 'completed', 'cancelled', 'revoke'],
      allowNull: false,
    },
    created_at: {
      type: DataTypes.DATE,
...
export const getOrders = () => Order.findAll({include:[Address,User,Delivery,Restaurant,  {
  model: OrderItem,
  include: [MenuItem]
}]});
export const getOrderById = (id: number) => Order.findByPk(id,{include:[Address,User,Delivery,Restaurant,
  {
    model: OrderItem,
    include: [MenuItem]
  }
]});
export const getOrdersByUserId = (user_id: number) => Order.findAll({ where: { user_id },include:[Address,User,Delivery,Restaurant,  {
  model: OrderItem,
  include: [MenuItem]
}]} );
...
export const getOrdersByPriceRange = (min: number, max: number) => Order.findAll({ where: { total_price: { [Op.between]: [min, max] } } });
export const getOrdersByStatus = (status: string) => Order.findAll({ where: { status } });
