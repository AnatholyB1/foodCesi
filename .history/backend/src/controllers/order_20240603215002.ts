...
import { askRestaurantForDelivery } from "./restaurant";
...
export const createAnOrder = withLogging('createAnOrder',async(req: express.Request, res: express.Response) => {
    try {
        ...
        if(!restaurant_id || !user_id || !address_id || !items || items.length === 0){
        ...
        const order_item : OrderItem[]  = items.forEach(async (item :any) =>
            {
        ...
        const restaurant_decision = await askRestaurantForDelivery(restaurant_id, address_id, order_item)
        //check validation here

        const availableDeliveries = await getAvailableDeliveriesByCity(address_id)

        if(!availableDeliveries){
            return res.status(400).json({message: 'No available deliveries'})
        }
