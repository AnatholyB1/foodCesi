...
import {
  ...
  addCategory,
  GetCategoriesWithMenuItemsByRestaurantId
} from "../db/menu_items";
...
export const getAllCategoriesWithMenuItemsByRestaurantId = withLogging(
  "getCategoriesWithMenuItemsByRestaurantId",
  async (req: express.Request, res: express.Response) => {
    try {
      const { restaurant_id } = req.params;
      if (!restaurant_id) return res.status(400).end();
      const items = await GetCategoriesWithMenuItemsByRestaurantId(parseInt(restaurant_id));
      if (!items) return res.status(404).end();
      return res.status(200).json(items).end();
    } catch (e) {
      return res.status(500).json(e).end();
